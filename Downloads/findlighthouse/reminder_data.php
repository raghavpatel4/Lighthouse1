...
if (!empty($_REQUEST['year']) && !empty($_REQUEST['month'])) {
    ...
    for ($i = 0; $i < $lastday; $i++) {
		...
		$date = $year . '-' . str_pad($month, 2, '0', STR_PAD_LEFT) . '-' . str_pad($i+1, 2, '0', STR_PAD_LEFT);
		
		$msgArr = getCustomResult("select * from `reminders` WHERE `user_id` = '".$_SESSION["user_id"]."' AND `re_date` = '".$date."' order by `reminder_id` desc");
		if(count($msgArr) > 0) {
			$dates[$data] = array(
				'date' => $date,
				//'badge' => ($i & 1) ? true : false,
				'badge' => false,
				'title' => trim($msgArr[0]['title']). ' ' . $date,
				'body' => '<p class="lead">'.trim($msgArr[0]['description']).'</p>',
				'footer' => '',
			);
			$data++;
		}

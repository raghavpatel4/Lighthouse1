	...
	if(isset($_POST)) {
		...
		if(count($resultArr) > 0) {
	    	...
	    	foreach($resultArr as $keys => $vals) {
				...
				$checkLoginStatus = getRowResult(trim('vpb_online_users'), " WHERE `user_id` = '".$vals['id']."'");
				if(count($checkLoginStatus) == 0) {
					$insertStatus = array(
						'lastTime' => date('Y-m-d h:i:s'),
						'lastTimeStr' => time(),
						'user_id' => $vals['id']
					);
					dbRowInsert(trim('vpb_online_users'), $insertStatus);
				} else {
					$insertStatus = array(
						'lastTime' => date('Y-m-d h:i:s'),
						'lastTimeStr' => time()
					);
					$resturnVal = dbRowUpdate(trim('vpb_online_users'), $insertStatus, " WHERE `user_id` = '".$vals['id']."'");
				}
			}
